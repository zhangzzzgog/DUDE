...
class ReActAgent:
    def __init__(
        ...
        参数�?
        - tools: 提供�?LLM 调用的工具函数列�?
        - model: 模型名称（例�?"glm-4.6v" �?"Qwen/Qwen3-VL-4B-Instruct"�?
        - project_directory: 当前项目目录
        - backend: "glm" 使用远程 GLM API�?qwen3_local" 使用本地 Qwen3-VL 后端
        - device: 本地模型使用的设备（�?"cuda" �?"cpu"），为空则由后端自行选择
        """
    ...
    def run(self, user_input: str|None=None, image_paths: List[str]|None=None, max_steps: int=3):
        ...
        while True:
            ...
            # 检测模型是否输�?Final Answer，如果是的话，直接返�?
